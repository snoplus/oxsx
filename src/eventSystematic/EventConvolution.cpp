...
EventConvolution::EventConvolution(const EventConvolution &other_)
{
  if (other_.fDist)
    ...
    fDist = NULL;
  fInObservables = other_.fInObservables;
  fOutObservables = other_.fOutObservables;
}
...
void EventConvolution::SetPDF(PDF *f_)
{
  if (!f_)
    fDist = NULL;
...
void EventConvolution::SetConditionalPDF(ConditionalPDF *c_)
{
  if (!c_)
    fDist = NULL;
  else
...
void EventConvolution::RenameParameter(const std::string &old_, const std::string &new_)
{
  if (!fDist)
...
void EventConvolution::SetParameter(const std::string &name_, double value_)
{
  if (!fDist)
    throw NULLPointerAccessError("EventConvolution::GetParameterNames",
                                 "Have you set the sampling function?");
...
void EventConvolution::SetParameters(const ParameterDict &ps_)
{
  if (!fDist)
    throw NULLPointerAccessError("EventConvolution::GetParameterNames",
                                 "Have you set the sampling function?");
...
void EventConvolution::SetName(const std::string &n_)
{
  fName = n_;
}

// Event Systematic Interface
Event EventConvolution::operator()(const Event &event_)
{
  if (!fDist)
    throw NULLPointerAccessError("EventConvolution::operator()",
                                 "Have you set the sampling function?");

  std::string obsToActOn = fOutObservables.GetNames().at(0);
  // change below once we support more than 1D
  double newVal = fDist->Sample(event_.ToObsSet(fOutObservables)).at(0);

  Event newEv(event_);
  newEv.SetDatum(obsToActOn, newVal);
