...
Event EventScale::operator()(const Event &inEvent_)
{
...
void EventScale::SetScale(double scale_)
{
    if (scale_ <= 0)
...
void EventScale::SetParameter(const std::string &name_, double value)
{
    if (name_ != fParamName)
...
void EventScale::SetParameters(const ParameterDict &pd_)
{
    try
    {
...
void EventScale::RenameParameter(const std::string &old_, const std::string &new_)
{
    if (old_ != fParamName)
...
void EventScale::SetName(const std::string &name_)
{
    fName = name_;
}
