...
void Shift::Construct()
{
    ...
    for (size_t bin = 0; bin < shiftAxis.GetNBins(); bin++)
    {
        ...
        for (size_t bin_test = shiftLowIndex; bin_test <= shiftHighIndex; bin_test++)
        {
            ...
            if (contribution > 0)
            {
                shift_vals[bin][bin_test] = contribution;
            }
        }
    }
...
void Shift::SetShift(double shift_)
{
    fShift = shift_;
}
...
void Shift::SetParameter(const std::string &name_, double value)
{
    if (name_ != fParamName)
...
void Shift::SetParameters(const ParameterDict &pd_)
{
    try
    {
...
void Shift::RenameParameter(const std::string &old_, const std::string &new_)
{
    if (old_ != fParamName)
...
void Shift::SetName(const std::string &name_)
{
    fName = name_;
}
