...
void Systematic::SetResponse(const SparseMatrix &responseMatrix_)
{
...
void Systematic::SetTransformationObs(const ObsSet &rep_)
{
    fTransObs = rep_;
}
...
void Systematic::SetDistributionObs(const ObsSet &rep_)
{
    fDistObs = rep_;
}
...
bool Systematic::BinsCompatible(size_t bin1_, size_t bin2_) const
{
...
bool Systematic::VectorContains(const std::vector<size_t> &vec_, size_t val_) const
{
...
void Systematic::SetAxes(const AxisCollection &axes_)
{
    fAxes = axes_;
