...
void SparseMatrix::SetComponent(size_t row_, size_t col_, double val_)
{
...
{
    if (!fNRows || !fNCols)
        throw DimensionError(Formatter() << "SparseMatrix:: Can't set elements to zero. (rows,cols) : (" << fNRows << "," << fNCols << ")");
    fArmaMat = arma::sp_mat(fNRows, fNCols);
}

void SparseMatrix::SetToIdentity()
{
    if (!fNRows || !fNCols || fNCols != fNRows)
        throw DimensionError(Formatter() << "SparseMatrix:: Can't set identity as matrix is not square. (rows,cols) : (" << fNRows << "," << fNCols << ")");
    fArmaMat.eye();
}

// FIXME: unsigned vs. size_t
void SparseMatrix::SetComponents(const std::vector<long long unsigned int> &rowIndices_,
