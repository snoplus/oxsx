...
void DenseMatrix::SetComponent(size_t row_, size_t col_, double val_)
{
...
void DenseMatrix::SetZeros()
{
    if (!fNRows || !fNCols)
        throw DimensionError(Formatter() << "DenseMatrix:: Can't set elements to zero. (rows,cols) : (" << fNRows << "," << fNCols << ")");
    fArmaMat.zeros();
}
...
void DenseMatrix::SetSymmetricMatrix(const std::vector<double> &_input)
{
