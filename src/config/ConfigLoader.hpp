...
void ConfigLoader::Load(const std::string &section_, const std::string &fieldName_, TargetType &loadVal_, typename enable_if<is_number<TargetType>::value, int>::type)
{
...
template <typename TargetType>
void ConfigLoader::Load(const std::string &section_, const std::string &fieldName_, TargetType &loadVal_, typename enable_if<is_container<TargetType>::value, int>::type)
{
  CheckExists(section_, fieldName_);
...
void ConvertContainer(const InContainer &incntr_, OutContainer &outcntr_, const ConverterSp &cnvtr_)
{
