...
public:
    ...
    QuadraticConstraint(double mean_, double width_) : fMean(mean_), fWidth(width_), fWidth_lo(0), is_asym(false) {}
    // Asymmetric constraint constructor
    QuadraticConstraint(double mean_, double width_hi, double width_lo) : fMean(mean_), fWidth(width_hi), fWidth_lo(width_lo), is_asym(true) {}
    ...
    double Evaluate(double val_) const
    {
        ...
        if (is_asym)
        {
            sigma = (val_ >= fMean) ? fWidth : fWidth_lo;
        }
        else
        {
            sigma = fWidth;
