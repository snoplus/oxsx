...
class BivariateQuadraticConstraint{
 public:
    ...
    // Main constructor
    BivariateQuadraticConstraint(double mean_1, double sigma_1, double mean_2, double sigma_2, double correlation) : 
        fMean1(mean_1), fSigma1(sigma_1), fMean2(mean_2), fSigma2(sigma_2), fCorr(correlation) {
            if (fCorr == 1.) {
