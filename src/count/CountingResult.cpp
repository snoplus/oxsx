...
void CountingResult::AddBackground(double expectedRate_, const std::string &name_,
...
void CountingResult::SetDataLog(const CutLog &log_)
{
    fDataLog = log_;
}

void CountingResult::SetObservedCounts(int counts_)
{
...
int CountingResult::GetObservedCounts() const
{
    return fObservedCounts;
}

void CountingResult::SetSignal(double sigEff_, const std::string &name_, const CutLog &log_)
{
    fSignalName = name_;
    fSignalLog = log_;
...
void CountingResult::SaveAs(const std::string &fileName_)
{
    time_t t = time(0);
