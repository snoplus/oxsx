...
void CountingExperiment::CountData(DataSet *testData_)
{
    int dataCount = 0;
...
void CountingExperiment::CountBackgrounds()
{
...
void CountingExperiment::CountSignal()
{
    int signalCount = 0;
    CutLog cutLog = (fCuts.GetCutNames());
...
void CountingExperiment::AddCut(const Cut &c_)
{
    fCuts.AddCut(c_);
}

void CountingExperiment::AddSystematic(EventSystematic *sys_, const std::string &name_)
{
    fSystematics.Add(sys_);
    fSystematicNames.push_back(name_);
}

void CountingExperiment::AddBackground(DataSet *mcData_, double rate_, const std::string &name_)
{
...
void CountingExperiment::SetSignal(DataSet *mcData_, const std::string &name_)
{
