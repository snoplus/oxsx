...
TEST_CASE("SparseMatrix tests")
{
    ...
    SECTION("Set components, all-in-one")
    {
        const std::vector<long long unsigned int> r_idxs {0, 1, 0, 0};
        const std::vector<long long unsigned int> c_idxs {0, 0, 1, 2};
        const std::vector<double> vals {1.3, 5, 10, 2};
        A.SetComponents(r_idxs, c_idxs, vals);

        REQUIRE(A.GetComponent(0, 0) == 1.3);
        REQUIRE(A.GetComponent(1, 0) == 5);
        REQUIRE(A.GetComponent(0, 1) == 10);
        REQUIRE(A.GetComponent(0, 2) == 2);
    }
    ...
    SECTION("Scale (square) matrix")
    {
        B.SetToIdentity();
        B.Scale(5.3);
        REQUIRE_THAT(B.GetComponent(0, 0), Catch::Matchers::WithinAbs(5.3, 0.00001));
        REQUIRE(B.GetComponent(1, 0) == 0);
        REQUIRE(B.GetComponent(0, 1) == 0);
        REQUIRE_THAT(B.GetComponent(1, 1), Catch::Matchers::WithinAbs(5.3, 0.00001));
    }
    ...
    SECTION("Multiply two matrices")
    {
        ...
        REQUIRE(C.GetNCols() == 3);
        REQUIRE_THAT(C.GetComponent(0, 0), Catch::Matchers::WithinAbs(2.*1+0*1, 0.00001));
        REQUIRE_THAT(C.GetComponent(0, 1), Catch::Matchers::WithinAbs(2.*1+0*1, 0.00001));
        REQUIRE_THAT(C.GetComponent(0, 2), Catch::Matchers::WithinAbs(2.*1+0*1, 0.00001));
        REQUIRE_THAT(C.GetComponent(1, 0), Catch::Matchers::WithinAbs(1.*1+0*0, 0.00001));
        REQUIRE_THAT(C.GetComponent(1, 1), Catch::Matchers::WithinAbs(1.*1+0*0, 0.00001));
        REQUIRE_THAT(C.GetComponent(1, 2), Catch::Matchers::WithinAbs(1.*1+0*0, 0.00001));

        REQUIRE(B.GetNRows() == 2);
        REQUIRE(B.GetNCols() == 2);
        REQUIRE_THAT(B.GetComponent(0, 0), Catch::Matchers::WithinAbs(2, 0.00001));
        REQUIRE_THAT(B.GetComponent(0, 1), Catch::Matchers::WithinAbs(0, 0.00001));
        REQUIRE_THAT(B.GetComponent(1, 0), Catch::Matchers::WithinAbs(1, 0.00001));
        REQUIRE_THAT(B.GetComponent(1, 1), Catch::Matchers::WithinAbs(0, 0.00001));
    }
