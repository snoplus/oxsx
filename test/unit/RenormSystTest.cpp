...
TEST_CASE("Simple renormalisation systematic on 1d PDF")
{
    AxisCollection axes;
    ...
    pdf1.SetBinContent(0, 5);
    pdf1.SetBinContent(1, 12);
    pdf1.SetBinContent(2, 10);
    pdf1.SetBinContent(3, 8);
    pdf1.SetBinContent(4, 3);
    pdf1.SetObservables(observables);

    Renorm *renorm = new Renorm("renorm");
    renorm->SetScaleFactor(2.0);
    renorm->SetAxes(axes);
    renorm->Construct();
    ...
    correctVals.push_back(24);
    correctVals.push_back(20);
    correctVals.push_back(16);
    correctVals.push_back(6);

    REQUIRE(modifiedObs == correctVals);
}

TEST_CASE("Another simple renormalisation systematic on 2d PDF")
{
    AxisCollection axes;
    axes.AddAxis(BinAxis("axis0", 0, 5, 5));
    ...
    correctVals[axes.FlattenIndices({1, 0})] = 12.1;
    correctVals[axes.FlattenIndices({2, 1})] = 24.2;
...
TEST_CASE("Another simple renormalisation systematic on 3d PDF")
{
    AxisCollection axes;
    axes.AddAxis(BinAxis("axis0", 0, 5, 5));
    axes.AddAxis(BinAxis("axis1", 1, 3, 2));
    ...
    correctVals[axes.FlattenIndices({1, 0, 5})] = 5.5;
    correctVals[axes.FlattenIndices({2, 1, 3})] = 11.0;
