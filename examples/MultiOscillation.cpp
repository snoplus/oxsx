...
const std::string dataFile = "/data/snoplus/blakei/antinu/mc/ntuples/test/OscBruceflux1000ds21_7.4e-05_ss12_0.297_ss13_0.0215_oxsx.root";
...
void LHFit(){
  ...
  //want to be able to declare these iteravely: UnOsc1 UnOsc2 UnOsc3 etc..
  ...
  //BinnedED OtherUnOscPdf("OtherUnOscPdf",axes);
  
  /*std::vector<BinnedED> ReactorPdfs;
  ReactorPdfs.push_back(BinnedED BruceUnOscPdf("BruceUnOscPdf",axes));
  ReactorPdfs.push_back(BinnedED DarlingtonUnOscPdf("DarlingtonUnOscPdf",axes));
  ReactorPdfs.push_back(BinnedED PickeringUnOscPdf("PickeringUnOscPdf",axes));
  ReactorPdfs.push_back(BinnedED OtherUnOscPdf("OtherUnOscPdf",axes));*/
  //BinnedED ReactorPdfs[] = {BruceUnOscPdf("BruceUnOscPdf",axes), DarlingtonUnOscPdf("DarlingtonUnOscPdf",axes),PickeringUnOscPdf("PickeringUnOscPdf",axes), OtherUnOscPdf("OtherUnOscPdf",axes)}
  
  char name[100];
  BinnedED *ReactorOscPdfs[4];
  std::vector<BinnedED> mcPdfs;
  for (int i = 0; i< 4; i++){
    ROOTNtuple UnOscNtp(UnOscfile, UnOscTreeName);
    sprintf(name,"ReactorOscPdf%d",i);
    ReactorOscPdfs[i] = new BinnedED(name,axes);
  ...
  for (int i = 0; i < numPdfs; i++){
    ReactorPdfs[i].SetObservables(0);
    for(size_t i = 0; i < UnOscNtp.GetNEntries(); i++){
      ReactorPdfs[i].Fill(UnOscNtp.GetEntry(i));
    }
    ReactorPdfs[i].Normalise();
    mcPdfs.push_back(ReactorPdfs[i]);
  } 
