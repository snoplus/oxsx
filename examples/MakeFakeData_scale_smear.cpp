	...
	// reset branch addresses after oscillating in function (otherwise crash before setting again below..)
        ...
        if (apply_oscillation || is_further_reactors){
          ...
          if (apply_energy_scaling && apply_energy_resolution_convolution) {
            ...
            reactors_data_set_pdf.Add(pdf_escale_eres);
          ...
          if (apply_energy_scaling && apply_energy_resolution_convolution) {
            ...
            BinnedED pdf_escale_eres = dataconv->operator()(pdf_escale);
            pdf_escale_eres.Normalise();
            pdf_escale_eres.Scale(constraint_osc_mean*geo_uth_custom_flux);
            data_set_pdf.Add(pdf_escale_eres);
            geos_data_set_pdf.Add(pdf_escale_eres);
          }else if (apply_energy_scaling && !apply_energy_resolution_convolution){
            BinnedED pdf_escale = datascale->operator()(*reactor_osc_pdf[i]);
            pdf_escale.Normalise();
            pdf_escale.Scale(constraint_osc_mean*geo_uth_custom_flux);
            data_set_pdf.Add(pdf_escale);
            geos_data_set_pdf.Add(pdf_escale);
          }else if (!apply_energy_scaling && apply_energy_resolution_convolution){
            BinnedED pdf_eres = dataconv->operator()(*reactor_osc_pdf[i]);
            pdf_eres.Normalise();
            pdf_eres.Scale(constraint_osc_mean*geo_uth_custom_flux);
            data_set_pdf.Add(pdf_eres);
            geos_data_set_pdf.Add(pdf_eres);
          }else{
            reactor_osc_pdf[i]->Normalise(); 
            reactor_osc_pdf[i]->Scale(constraint_osc_mean*geo_uth_custom_flux);
            data_set_pdf.Add(*reactor_osc_pdf[i]);
          ...
          if (apply_energy_scaling && apply_energy_resolution_convolution) {
            ...
            pdf_escale_eres.Scale(constraint_osc_mean);
            data_set_pdf.Add(pdf_escale_eres);
            alpha_n_data_set_pdf.Add(pdf_escale_eres);
            if (split_alpha_n_pdf_1st)
              alpha_n_1st_data_set_pdf.Add(pdf_escale_eres);
            else if (split_alpha_n_pdf_2nd)
              alpha_n_2nd_data_set_pdf.Add(pdf_escale_eres);
          }else if (apply_energy_scaling && !apply_energy_resolution_convolution){
            BinnedED pdf_escale = datascale->operator()(*reactor_osc_pdf[i]);
            pdf_escale.Normalise();
            pdf_escale.Scale(constraint_osc_mean);
            data_set_pdf.Add(pdf_escale);
            alpha_n_data_set_pdf.Add(pdf_escale);
            if (split_alpha_n_pdf_1st)
              alpha_n_1st_data_set_pdf.Add(pdf_escale);
            else if (split_alpha_n_pdf_2nd)
              alpha_n_2nd_data_set_pdf.Add(pdf_escale);
          }else if (!apply_energy_scaling && apply_energy_resolution_convolution){
            BinnedED pdf_eres = dataconv->operator()(*reactor_osc_pdf[i]);
            pdf_eres.Normalise();
            pdf_eres.Scale(constraint_osc_mean);
            data_set_pdf.Add(pdf_eres);
            alpha_n_data_set_pdf.Add(pdf_eres);
            if (split_alpha_n_pdf_1st)
              alpha_n_1st_data_set_pdf.Add(pdf_eres);
            else if (split_alpha_n_pdf_2nd)
              alpha_n_2nd_data_set_pdf.Add(pdf_eres);
          }else{
            reactor_osc_pdf[i]->Normalise(); 
            reactor_osc_pdf[i]->Scale(constraint_osc_mean);
            data_set_pdf.Add(*reactor_osc_pdf[i]);
