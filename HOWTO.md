...
a name (e.g. "energy"), and can also have an optional name to be used for pretty LaTeX formatting ("E\_{reco}", for example).
...
mathematical function: it can be _evaluated_ at some set of values given to it,
...
Let's go another level deeper! a `PDF` is _another_ abstract class, which is a
...
foreach dataset _before_ cuts. More on cuts in OXO [here](#61-cut).
...
to know is whether a certain event passes _all_ the cuts specified: this is
